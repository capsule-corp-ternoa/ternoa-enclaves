...
fn is_user_report_data_exist(file_path: Option<String>) -> bool {
	...
	// if Some(file_path.clone()) {
	// 	Path::new(file_path.unwrap().as_str()).exists()
	// } else {
	// 	Path::new("/dev/attestation/user_report_data").exists()
	// }
}
...
// 	info!("This is inside Enclave!");
//
// 	let mut f2 = File::open("/dev/attestation/attestation.attestation_type").unwrap(); // TODO: manage unwrap()
// 	let mut attest_type = String::new();
