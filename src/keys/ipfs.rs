...
use std::{env::var, io::Read, path::Path};
...
impl TernoaIpfsApi {
	pub fn new(apiurl: &String, gateurl: &String) -> Self {
		TernoaIpfsApi { api_url: apiurl.to_string(), gateway_url: gateurl.to_string() }
	}
...
mod tests {
	...
	async fn download_file_http() {
		...
		let ternoa_ipfs =
			TernoaIpfsApi::new(&(IPFS_GATEWAY_URL.to_string()), &IPFS_GATEWAY_URL.to_string());
